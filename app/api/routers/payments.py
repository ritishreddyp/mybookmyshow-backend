...
def get_checkout(booking_id: UUID , db: Session = Depends(require_role(["public"])),  current_user: SQUser = Depends(require_role(["public", "theater_admin", "admin"]))):
...
def initiate_txn(payload: PaymentProcessRequest, db: Session = Depends(get_db), current_user: SQUser = Depends(require_role(["public"]))):
    return initiate_payment(payload=payload, user_id=current_user.id, db=db)


@router.post("/verify")
def verify_txn( booking_id: UUID , transaction_id: str,  db: Session = Depends(get_db), current_user: SQUser = Depends(require_role(["public"]))):
