...
def generate_seats(hall : SeatCreate, db: Session = Depends(get_db),current_user = Depends(require_role(["theater_admin", "admin"]))):
    return generate_seats_for_screen(hall, db)

@router.delete("/theater/{theater_id}/screen/{screen_id}/seats")
def remove_screen_seats(theater_id: UUID, screen_id: UUID, db: Session = Depends(get_db),current_user = Depends(require_role(["theater_admin", "admin"]))):
    return delete_seats_for_screen(theater_id, screen_id, db)

@router.patch("/theater/{theater_id}/screen/{screen_id}/seats-type")
def modify_screen_seats_type(theater_id: UUID, screen_id: UUID, new_seat_type: str, db: Session =  Depends(get_db),current_user =Depends(require_role(["theater_admin", "admin"]))):
