...
from app.crud import get_all_cities,get_city_id,create_city,delete_city,restore_city,get_inactive_cities
...
@router.get("/city/inactive")
def list_inactive_cities(db: Session = Depends(get_db)):
    return get_inactive_cities(db)

@router.patch("/{city_id}/activate")
def activate_city(city_id: int, db: Session = Depends(get_db)):
