...
from app.crud import create_language,assign_languages_to_movie,update_language,delete_language,get_languages_by_movie,get_movies_by_language,remove_language_from_movie, get_inactive_languages, activate_language
...
@router.get("/languages/inactive")
def list_inactive_languages(db: Session = Depends(get_db)):
    return get_inactive_languages(db)

@router.patch("/{language_id}/activate")
def restore_language(language_id: int, db: Session = Depends(get_db)):
